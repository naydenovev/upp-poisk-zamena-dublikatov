...
Функция ПометитьДублиНаУдалениеИУдалить(МассивСсылок) Экспорт
	...
	Если РезультатТиповогоУдаления.Значение.Найденные.Количество() > 0 Тогда
		...
		Для Каждого СтрокаНайденных Из РезультатТиповогоУдаления.Значение.Найденные Цикл
			Если ТекстПричин <> "" Тогда
				ТекстПричин = ТекстПричин + Символы.ПС;
			КонецЕсли;
			ТекстПричин = ТекстПричин + Строка(СтрокаНайденных.Ссылка) + " <- " + Строка(СтрокаНайденных.Данные) + " (" + Строка(СтрокаНайденных.Метаданные) + ")";
		КонецЦикла;
		Результат.ТекстПричин = ТекстПричин;
	КонецЕсли;
...
Процедура ПоказатьОтчетУдаленияДублей(Результат)
	...
	Если Результат.НеУдаленные.Количество() = 0 И ПустаяСтрока(Результат.ТекстПричин) Тогда
		Предупреждение(НСтр("ru='Помечено на удаление: ';uk='Позначено на видалення: '") + Результат.Помечено
			+ НСтр("ru='. Удалено: ';uk='. Видалено: '") + Результат.Удалено + ".");
		Возврат;
	КонецЕсли;

	ТекстОтчета = НСтр("ru='Помечено на удаление: ';uk='Позначено на видалення: '") + Результат.Помечено + Символы.ПС
		+ НСтр("ru='Удалено: ';uk='Видалено: '") + Результат.Удалено + Символы.ПС
		+ НСтр("ru='Не удалено: ';uk='Не видалено: '") + Результат.НеУдаленные.Количество();
...
Процедура КоманднаяПанельНайденныеСсылкиУдалитьЭлементы(Кнопка)
	...
	Если МассивСтрок.Количество() > 0 Тогда
		...
		ТекстВопроса = НСтр("ru='Будет помечено на удаление и проверено на возможность удаления ';uk='Буде позначено на видалення і перевірено на можливість видалення '") + МассивСтрок.Количество()
			+ НСтр("ru=' эл-тов группы. Продолжить?';uk=' ел-тів групи. Продовжити?'");
