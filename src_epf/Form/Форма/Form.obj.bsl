...
Процедура ОткрытьФормуПоНастройке(ДанныеНастройки, Элемент)
	...
	ЕСли ДанныеНастройки.Предопределенный Тогда	
		...
		ИначеЕсли ДанныеНастройки.ИмяНастройки = "Режим поиска" Тогда
			...
			СтрокаОтбора = ЗначенияОтбора.Добавить();
			СтрокаОтбора.Представление = НСтр("ru='По равному значению реквизита';uk='По рівному значення реквізиту'");
			СтрокаОтбора.Значение = 1;
			СтрокаОтбора.ИндексКартинки = -1;
			
			СтрокаОтбора = ЗначенияОтбора.Добавить();
			СтрокаОтбора.Представление = НСтр("ru='По похожим словам';uk='За схожим словами'");
	...
	Иначе
		...
		Если МетаданныеСправочника.Владельцы.Количество()<>0 Тогда
			СтрокаОтбора = ЗначенияОтбора.Добавить();
			СтрокаОтбора.Представление = НСтр("ru='Владелец';uk='Власник'");
		...
		Если МетаданныеСправочника.Иерархический Тогда
			СтрокаОтбора = ЗначенияОтбора.Добавить();
			СтрокаОтбора.Представление = НСтр("ru='Родитель';uk='Батьківський елемент'");
		...
		Если МетаданныеСправочника.ДлинаНаименования > 0 Тогда
			СтрокаОтбора = ЗначенияОтбора.Добавить();
			СтрокаОтбора.Представление = НСтр("ru='Наименование';uk='Найменування'");
		...
		Если МетаданныеСправочника.ДлинаКода > 0 Тогда
			СтрокаОтбора = ЗначенияОтбора.Добавить();
			СтрокаОтбора.Представление = НСтр("ru='Код';uk='Код'");
...
Процедура НайтиПоСовпадающимСловам(ТаблицаОтбора)
	...
	ЕСли мФормаИндикатора<>Неопределено Тогда
		мФормаИндикатора.КомментарийОбработкиДанных = НСтр("ru='Поиск элементов';uk='Пошук елементів'");
		мФормаИндикатора.КомментарийЗначения = НСтр("ru='Получение элементов справочника';uk='Отримання елементів довідника'");
	...
	ЕСли мФормаИндикатора<>Неопределено Тогда
		мФормаИндикатора.КомментарийОбработкиДанных = НСтр("ru='Разбивка значения на отдельные слова';uk='Розбивка значення на окремі слова'");
	...
	ЕСли мФормаИндикатора<>Неопределено Тогда
		мФормаИндикатора.КомментарийОбработкиДанных = НСтр("ru='Выполняется анализ значений реквизита';uk='Виконується аналіз значень реквізиту'");
	...
	ЕСли мФормаИндикатора<>Неопределено Тогда
		мФормаИндикатора.КомментарийОбработкиДанных = НСтр("ru='Обработка результатов';uk='Обробка результатів'");
	КонецЕсли;
...
Функция ВыполнитьПоискСсылок()
	...
	Если СтрокаПравильного = Неопределено Тогда
		Предупреждение(НСтр("ru='Не определен правильный элемент';uk='Не визначено правильний елемент'"));
		Возврат ложь;
...
Функция ВыполнитьЗаменуЭлементов(ПравильныйЭлемент, ТаблицаСсылок, ЗаголовокИндикации = "")
	...
	ЕСли мФормаИндикатора<>Неопределено Тогда
		...
		мФормаИндикатора.КомментарийЗначения = НСтр("ru='Выполняется замена элементов.';uk='Виконується заміна елементів.'") + ЗаголовокИндикации;
		мФормаИндикатора.Значение = 0;
	КонецЕсли;
	...
	Для Каждого СтрокаТаблицы Из ТаблицаСсылок Цикл
		...
		Иначе
			Сообщить(НСтр("ru='Ссылки типа ';uk='Посилання типу '")+СтрокаТаблицы.Метаданные+НСтр("ru=' не заменяются!!';uk=' не замінюються!!'"));
		КонецЕсли;
		ОбработкаПрерыванияПользователя();
	КонецЦикла;
...
Процедура АвтозаменаЭлементов(НепосредственноУдалятьОбъекты) Экспорт
	...
	ЕСли мФормаИндикатора<>Неопределено Тогда
		мФормаИндикатора.КомментарийОбработкиДанных = НСтр("ru='Поиск и замена значений';uk='Пошук і заміна значень'");
		мФормаИндикатора.ВладелецФормы = ЭтаФорма;
		мФормаИндикатора.Открыть();
	КонецЕсли;
	...
	Для Каждого СтрокаПравильного Из ПравильныеСтроки Цикл
		...
		ЕСли мФормаИндикатора<>Неопределено Тогда
			мФормаИндикатора.КомментарийЗначения = НСтр("ru='Выполняется поиск ссылок.';uk='Виконується пошук посилань.'") + ЗаголовокИндикации;
			мФормаИндикатора.Значение = 0;
		КонецЕсли;
...
Процедура ОчиститьРезультатыПоиска()
	...
	НайденныеОбъекты.Колонки.Добавить("КоличествоСсылок", , НСтр("ru='Количество ссылок';uk='Кількість посилань'"));
...
Процедура КоманднаяПанельНастройкиОтчетаПоиск(Кнопка)
	...
	Если ПустаяСтрока(ПолучитьЗначениеНастройки("Имя реквизита")) Тогда
		Предупреждение(НСтр("ru='Не указаны все необходимые параметры поиска';uk='Не вказані всі необхідні параметри пошуку'"));
		Возврат;
	КонецЕсли;
	...
	Иначе
		Предупреждение(НСтр("ru='Не указан режим поиска';uk='Не вказаний режим пошуку'"));
		Возврат;
	КонецЕсли;
	...
	ЕСли ТипЗнч(ВыбранноеЗначение) = Тип("Структура") Тогда
		...
		Если ЭлементыФормы.НастройкиОтчета.ТекущиеДанные.ИмяНастройки = "Режим поиска"
			...
			Если СбросТипа Тогда
				Предупреждение(НСтр("ru='Данный режим поиска имеет смысл выполнять только для реквизитов строкового типа';uk='Даний режим пошуку має сенс виконувати тільки для реквізитів рядкового типу'"));
				Возврат;
			КонецЕсли;
		КонецЕсли;
...
Процедура КоманднаяПанельНайденныеОбъектыАвтозамена(Кнопка)
	...
	Если мРезультатыПоиска = Неопределено или Не мРезультатыПоиска.НайтиСтроки(Новый Структура("Правильный",Истина)).Количество() Тогда
		Предупреждение(НСтр("ru='Не определен правильный элемент ни в одной из групп.';uk='Не визначено правильний елемент в жодній з груп.'"));
		Возврат;
	КонецЕсли;
	
	РезультатВопроса = Вопрос(НСтр("ru='При автоматической замене будет произведен поиск и замена"
"неправильных элементов, в группах, где определен правильный элемент."
""
"Выполнить непосредственное удаление объектов после замены элементов?';uk=""При автоматичній заміні буде проведений пошук і заміна"
"неправильних елементів, у групах, де визначений правильний елемент."
""
"Виконати безпосереднє вилучення об'єктів після заміни елементів?"""), РежимДиалогаВопрос.ДаНетОтмена);
...
Процедура НайденныеОбъектыПриОкончанииРедактирования(Элемент, НоваяСтрока, ОтменаРедактирования)
	...
	Если НайденныеОбъекты.НайтиСтроки(Новый Структура("Ссылка",ТекущаяСсылка)).Количество() > 1 Тогда
		Предупреждение(НСтр("ru='Элемент ""';uk='Елемент ""'") + ТекущаяСсылка + НСтр("ru='"" уже присутствует в списке!';uk='"" вже присутня у списку!'"));
...
Функция ПометитьДублиНаУдалениеИУдалить(МассивСсылок) Экспорт

	Результат = Новый Структура("Помечено, Удалено, Удаленные, НеУдаленные, ТекстПричин", 0, 0, Новый Массив, Новый Массив, "");

	Если Метаданные.Обработки.Найти("УдалениеПомеченныхОбъектов") = Неопределено Тогда
		Результат.ТекстПричин = НСтр("ru='В базе отсутствует типовая обработка ""Удаление помеченных объектов"". Удаление не выполнено.';uk='У базі відсутня типова обробка ""Видалення позначених об''єктів"". Видалення не виконано.'");
		Возврат Результат;
	КонецЕсли;
	...
	Для Каждого Ссылка Из МассивСсылок Цикл
		...
		Попытка
			Объект.Записать();
			Результат.Помечено = Результат.Помечено + 1;
		Исключение
			ОбщегоНазначения.СообщитьОбОшибке(ОписаниеОшибки());
		КонецПопытки;
	КонецЦикла;
	...
	РезультатТиповогоУдаления = Обработки.УдалениеПомеченныхОбъектов.УдалитьПомеченныеОбъекты(МассивСсылок, ТипыУдаленныхОбъектов);

	Если Не РезультатТиповогоУдаления.Статус Тогда
		Результат.ТекстПричин = РезультатТиповогоУдаления.Значение;
		Возврат Результат;
	КонецЕсли;

	НеУдаленные = РезультатТиповогоУдаления.Значение.НеУдаленные;
	Для Каждого Ссылка Из МассивСсылок Цикл
		Если НеУдаленные.Найти(Ссылка) = Неопределено Тогда
			Результат.Удалено = Результат.Удалено + 1;
			Результат.Удаленные.Добавить(Ссылка);
		Иначе
			Результат.НеУдаленные.Добавить(Ссылка);
		КонецЕсли;
	КонецЦикла;

	Если РезультатТиповогоУдаления.Значение.Найденные.Количество() > 0 Тогда
		СписокПричин = Новый Массив;
		Для Каждого СтрокаНайденных Из РезультатТиповогоУдаления.Значение.Найденные Цикл
			СписокПричин.Добавить(СтрШаблон("%1 <- %2 (%3)", СтрокаНайденных.Ссылка, СтрокаНайденных.Данные, СтрокаНайденных.Метаданные));
		КонецЦикла;
		Результат.ТекстПричин = СтрСоединить(СписокПричин, Символы.ПС);
	КонецЕсли;
...
Процедура ПоказатьОтчетУдаленияДублей(Результат)

	Если Результат.НеУдаленные.Количество() = 0 И ПустаяСтрока(Результат.ТекстПричин) Тогда
		Предупреждение(СтрШаблон(НСтр("ru='Помечено на удаление: %1. Удалено: %2.';uk='Позначено на видалення: %1. Видалено: %2.'"), Результат.Помечено, Результат.Удалено));
		Возврат;
	КонецЕсли;

	ТекстОтчета = СтрШаблон(НСтр("ru='Помечено на удаление: %1';uk='Позначено на видалення: %1'"), Результат.Помечено) + Символы.ПС
		+ СтрШаблон(НСтр("ru='Удалено: %1';uk='Видалено: %1'"), Результат.Удалено) + Символы.ПС
		+ СтрШаблон(НСтр("ru='Не удалено: %1';uk='Не видалено: %1'"), Результат.НеУдаленные.Количество());

	Если Не ПустаяСтрока(Результат.ТекстПричин) Тогда
		ТекстОтчета = ТекстОтчета + Символы.ПС + Символы.ПС + НСтр("ru='Причины (кто ссылается):';uk='Причини (хто посилається):'") + Символы.ПС + Результат.ТекстПричин;
	КонецЕсли;

	ДокументОтчета = Новый ТекстовыйДокумент;
	ДокументОтчета.УстановитьТекст(ТекстОтчета);
	ДокументОтчета.Показать(НСтр("ru='Результат удаления дублей';uk='Результат видалення дублів'"));
...
Процедура КоманднаяПанельНайденныеСсылкиУдалитьЭлементы(Кнопка)
	...
	Если НайденныеСсылки.Количество()>0 Тогда
		Предупреждение(НСтр("ru='На элементы есть ссылки';uk='На елементи є посилання'"));
		Возврат;
	КонецЕсли;
	...
	РезультатУдаленияДублей = Неопределено;

	Если МассивСтрок.Количество() > 0 Тогда

		ТекстВопроса = СтрШаблон(НСтр("ru='Будет помечено на удаление и проверено на возможность удаления %1 эл-тов группы. Продолжить?';uk='Буде позначено на видалення і перевірено на можливість видалення %1 ел-тів групи. Продовжити?'"), МассивСтрок.Количество());

		Если Вопрос(ТекстВопроса, РежимДиалогаВопрос.ДаНет, , КодВозвратаДиалога.Нет) = КодВозвратаДиалога.Да Тогда

			МассивСсылокНаУдаление = Новый Массив;
			Для Каждого Строка Из МассивСтрок Цикл
				МассивСсылокНаУдаление.Добавить(Строка.Ссылка);
			КонецЦикла;

			РезультатУдаленияДублей = ПометитьДублиНаУдалениеИУдалить(МассивСсылокНаУдаление);

			СтруктураПоиска = Новый Структура("НомерГруппы, Ссылка", ТекущаяГруппа, Неопределено);
			Для Каждого Ссылка Из РезультатУдаленияДублей.Удаленные Цикл
				СтруктураПоиска.Ссылка = Ссылка;
				Для Каждого СтрокаРезультатовПоиска Из мРезультатыПоиска.НайтиСтроки(СтруктураПоиска) Цикл
					мРезультатыПоиска.Удалить(СтрокаРезультатовПоиска);
				КонецЦикла;
				Для Каждого СтрокаНайденных Из НайденныеОбъекты.НайтиСтроки(Новый Структура("Ссылка", Ссылка)) Цикл
					НайденныеОбъекты.Удалить(СтрокаНайденных);
				КонецЦикла;
			КонецЦикла;
	...
	Если РезультатУдаленияДублей <> Неопределено Тогда
		ПоказатьОтчетУдаленияДублей(РезультатУдаленияДублей);
	КонецЕсли;
